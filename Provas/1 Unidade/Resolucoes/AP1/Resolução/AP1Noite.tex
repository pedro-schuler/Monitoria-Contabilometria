...
\[\hat{\beta_{1}} = \frac{n \times \sum (X_{i}Y_{i}) - \sum Y_{i}\sum X_{i}}{n \times \sum X^2 - (\sum X_{i})^2}\]
Agora podemos utilizar os valores da tabela criada para calcular \(\beta_{1}\).
\[ \hat{\beta_{1}} = \frac{5 \times 675 - 92 \times 56}{5 \times 778 - (56)^2} = -2,35676 \]
\(\hat{\beta_{0}}\) pode ser definido como \(\beta_{0} = \bar{Y} - \beta_{1} \times \bar{X}\), logo:
\[ \hat{\beta_{0}} = \frac{92}{5} + 2,35676 \times \frac{56}{5} = 44,79576 \]

Agora que temos os valores de \(\hat{\beta_{0}}\) e \(\hat{\beta_{1}}\) podemos interpretar o seguinte: na ausência de processos (quando o número de processos \(= 0\)) o valor do ativo da empresa é, em média, de R\$ 44,79576 milhões de reais. O \(\hat{\beta_{1}}\) nos informa que, quando o número de processos aumenta em uma unidade, o valor do ativo decresce, em média, em R\$ 2,35676 milhões de reais.
...
Inicialmente calculamos o \(det(X'X)\)
\[det(X'X = 7780 + 2576 + 2576 - 3112 - 2645 - 6272 = 903 )\]
...
Após calculados os valores de \(\hat{\beta}\) podemos interpretar que: para empresas do setor de \textit{serviços} e que não possuem nenhum processo, o valor do ativo é, em média, de R\$ 46,39424 \((\hat{\beta_{0}})\) milhões de reais. Quando o número de processos aumenta em 1 unidade, o valor do ativo diminui, em média, em R\$ 2,33887 \((\hat{\beta_{1}})\) milhões de reais. As empresas do setor de \textit{comércio} têm, em média, um ativo R\$ 4,49723 \((\hat{\beta_{2}})\) milhões de reais menor que os das empresas do setor de \textit{serviços}.
...
\[ [\hat{\beta^{*}_{1}}-\hat{\beta^{*}_{1}} \times 25\%;\hat{\beta^{*}_{1}}+\hat{\beta^{*}_{1}} \times 25\%]\]
\[\hat{\beta^{*}_{1}} \times 25\% = -2,33887 \times \frac{25}{100} = -0,58472\]
\[[-2,33887 - 0,58472;-2,33887 + 0,58472]\]
\[[-2,92359;-1,75415]\]

O \(\hat{\beta_{1}}\) da regressão simples é igual a \(-2,35676\) e está, portanto, dentro do intervalo definido. Logo, concluímos que \textbf{não há} evidências de que \(H_{1}\) foi violada.
...
\[\hat{\beta_{0}} = 46,39424 \]
\[\hat{\beta_{1}} = -2,33887 \]
\[\hat{\beta_{2}} = -4,49723 \]

Primeiro calcularemos \(SQR\)
\[SQR = \sum (Y - \widehat{Y})^{2}\]
\[ [ 20 - (\hat{\beta_{0}} + \hat{\beta_{1}} \times 8  + \hat{\beta_{2}} \times 0)]^{2} =  59,03279 \]
\[ [ 40 - (\hat{\beta_{0}} + \hat{\beta_{1}} \times 5  + \hat{\beta_{2}} \times 0)]^{2} =  28,09117 \]
\[ [ 5  - (\hat{\beta_{0}} + \hat{\beta_{1}} \times 15 + \hat{\beta_{2}} \times 1)]^{2} =  3,29045 \]
\[ [ 25 - (\hat{\beta_{0}} + \hat{\beta_{1}} \times 8  + \hat{\beta_{2}} \times 1)]^{2} =  3,29041 \]
\[ [ 2  - (\hat{\beta_{0}} + \hat{\beta_{1}} \times 20 + \hat{\beta_{2}} \times 0)]^{2} =  5,67945 \]
\[ SQR = 99,38427\]
...
Para que seja possível utilizar o MQO precisamos atender \(H_{1}-H_{2}\). Se considerarmos que os fatores não observáveis não estão correlacionados com as variáveis explicativas, \(H_{1}\) estará atendida. Para tanto, basta que o modelo inclua uma constante \((\hat{\beta_{0}})\) e esta hipótese estará atendida.
